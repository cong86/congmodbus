...
RELOAD_GUARD_SECONDS = 4
SETUP_SAME_GENERATION_WINDOW_SECONDS = 3
RECOVERY_WARMUP_SECONDS = 20
RELOAD_WARNING_SUPPRESS_SECONDS = 90
DISCONNECTED_RETRY_SECONDS = 2
...
CONF_HUB = "hub"
DOMAIN = "congmodbus"
PYMODBUS_TID_REQUEST = "request ask for transaction_id="
PYMODBUS_TID_GOT = "but got id="
PYMODBUS_ORPHAN_PDU = "received pdu without a corresponding request, ignoring"


class _PymodbusTidMismatchFilter(logging.Filter):
    """Filter only noisy transaction id mismatch spam from pymodbus."""
    ...
    def filter(self, record):
        ...
        tid_mismatch = PYMODBUS_TID_REQUEST in message and PYMODBUS_TID_GOT in message
        return (
            not tid_mismatch
            and PYMODBUS_ORPHAN_PDU not in message
        )


def _install_pymodbus_filter_once(hass):
    key = "congmodbus_pymodbus_tid_filter_installed"
    if hass.data.get(key):
        return

    logging.getLogger("pymodbus.logging").addFilter(_PymodbusTidMismatchFilter())
    hass.data[key] = True
...
def setup_platform(hass, conf, add_devices, discovery_info=None):
    ...
    _install_pymodbus_filter_once(hass)
    ...
    if not entities:
        for prop in bus.regs:
            if CONF_REGISTER not in bus.regs[prop]:
                ...
                return
        entities.append(CongModbusClimate(bus, name[0] if isinstance(name, list) else name))

    bus.count = len(entities)
    status_name = bus.hub_name + " Polling"
    bus.ensure_status_sensor(status_name)
    switch_name = bus.hub_name + " Polling Switch"
    bus.ensure_polling_switch(switch_name)
...
class ClimateModbus:
    ...
    def __init__(self, hass, conf):
        ...
        hub_locks = self.hass.data.setdefault("congmodbus_io_locks", {})
        ...
        self._poll_runtime.setup_count += 1
        now = time.monotonic()
        if now - self._poll_runtime.last_setup_at > SETUP_SAME_GENERATION_WINDOW_SECONDS:
            self._poll_runtime.generation += 1
        self._poll_runtime.last_setup_at = now
        self._generation = self._poll_runtime.generation
        ...
        if self._generation > 1 and self._poll_runtime.guarded_generation != self._generation:
            self._poll_runtime.guarded_generation = self._generation
            self._guard_polling_after_reload()
    ...
    async def reconnect(self, now=None):
        ...
        _LOGGER.debug("Reconnect %s", self.hub._client)
        try:
    ...
    def _guard_polling_after_reload(self):
        """YAML 重载后短暂冷却并重连，尽量避免旧事务响应串入新请求。"""
        now = time.monotonic()
        self._poll_runtime.poll_paused = True
        self._poll_runtime.warmup_until = max(self._poll_runtime.warmup_until, now + RECOVERY_WARMUP_SECONDS)
        self._poll_runtime.suppress_warning_until = max(
            self._poll_runtime.suppress_warning_until,
            now + RELOAD_WARNING_SUPPRESS_SECONDS,
        )
        self._poll_runtime.next_poll_retry_at = max(
            self._poll_runtime.next_poll_retry_at,
            now + RELOAD_GUARD_SECONDS,
        )
        self._poll_runtime.next_poll_retry_wall = datetime.now() + timedelta(seconds=RELOAD_GUARD_SECONDS)
        _LOGGER.debug(
            "Detected YAML reload on %s, pause polling for %ss and restart hub",
            self.hub_name,
            RELOAD_GUARD_SECONDS,
        )

        if self._poll_runtime.reload_reconnect_pending:
            return

        self._poll_runtime.reload_reconnect_pending = True
        from homeassistant.helpers.event import async_call_later

        async_call_later(self.hass, 1, self._reload_reconnect)

    async def _reload_reconnect(self, now=None):
        try:
            await self.reconnect(now)
        finally:
            self._poll_runtime.reload_reconnect_pending = False
    ...
    def ensure_polling_switch(self, name):
        """确保轮询开关实体仅加载一次。"""
        if self._poll_runtime.switch_loaded:
            return

        self._poll_runtime.switch_loaded = True
        from homeassistant.helpers import discovery
    ...
    def should_poll(self):
        ...
        # YAML 重载后，让旧代实例永久停止轮询，避免新旧实例并发打同一 hub。
        if self._generation != self._poll_runtime.generation:
            return False
        ...
        if not self.is_transport_ready():
            self._poll_runtime.next_poll_retry_at = now + DISCONNECTED_RETRY_SECONDS
            return False

        self._poll_probe_in_progress = True
        return True

    def is_transport_ready(self):
        """Best-effort check to avoid issuing requests while transport is disconnected."""
        client = getattr(self.hub, "_client", None)
        if client is None:
            return True
    ...
    def in_probe_mode(self):
        if self._poll_runtime.poll_paused and self._poll_probe_in_progress:
            return True
        return time.monotonic() < self._poll_runtime.warmup_until

    def probe_entity_allowed(self, index):
        return index in (-1, 0)

    def probe_prop(self):
        for candidate in (REG_HVAC_MODE, REG_TEMPERATURE, REG_HVAC_OFF):
            if candidate in self.regs:
                return candidate
    ...
    def mark_poll_success(self):
        ...
        if self._poll_runtime.poll_paused:
            _LOGGER.debug("Modbus communication recovered on %s, resume polling", self.hub_name)
            self._poll_runtime.warmup_until = max(
                self._poll_runtime.warmup_until,
                time.monotonic() + RECOVERY_WARMUP_SECONDS,
            )
    ...
    def exception(self):
        ...
        if not self._poll_runtime.poll_paused:
            if now < self._poll_runtime.suppress_warning_until:
                _LOGGER.debug(
                    "Modbus communication failed on %s during reload stabilization, pause polling and retry in %ss",
                    self.hub_name,
                    delay,
                )
            elif now < self._poll_runtime.warmup_until:
                _LOGGER.debug(
                    "Modbus communication failed on %s during reload warmup, pause polling and retry in %ss",
                    self.hub_name,
                    delay,
                )
            else:
...
class CongModbusClimate(ClimateEntity):
    ...
    async def async_update(self):
        ...
        # 手动关闭轮询时保持实体可用，仅暂停读取设备。
        if not self._bus.polling_enabled():
        ...
        if self._bus.in_probe_mode() and not self._bus.is_transport_ready():
            return

        if self._bus.in_probe_mode() and not self._bus.probe_entity_allowed(self._index):
            return
